...
class ChatClient(CTk):
    def __init__(self):
        ...
        img = CTkImage(light_image=Imege.open("C:\Users\Honor\Desktop\Python\GUI\LogikaTalk\smooth-elegant-gradient-purple-background-well-using-as-design_1258-331.avif"))
        self.imege_ld=CTkLabel(self,text="",image=img)
        self.imege_ld.pack()
        ...
        self.nicknameframe.pack(pady=120)
        self.nickname_Ib = CTkLabel(
            self.nicknameframe,
        ...
        self.nickname_Ib.pack(pady=10,padx=10)
        ...
        self.chat_box.configure(text_color="blue",border_color="white")
    ...
    def connent_to_server(self):
        try:
            ...
            text = f"{self.nickname}@{self.nickname} приєднався до чату \n"
            client.send(text.encode())
            self.recv_thread = threading.Thread(target=self.recv_loop,deamon=True)
